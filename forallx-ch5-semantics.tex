...
Az SL esetében az igazság meghatározását két részre osztottuk: az igazságérték-hozzárendelést ($a$) a mondat betűire és az igazságfüggvényt ($v$) az összes mondatra. Az igazságfüggvény lefedte azt a módot, amellyel az összetett mondatokat betűkből és kötőszavakból lehet felépíteni.
...
Ugyanúgy, az SL \emph{az igazság igazságértéket adott meg} igazsága a QL \emph{az igazság egy modellben}  esetében is mindig igazság. A QL legegyszerűbb atom-mondata egy helyről származó állításból áll, amelyet egy állandó, például $Pj$ követ. Egy modellben \model{M} ez akkor és csak is akkor igaz, ha a $j$ referens a \model{M}-ben lévő $P$ kiterjesztésében található.
...
Ezután meghatározhatjuk az igazságot, melyek a mondathoz tartozó összeköttetésekkel épültek fel, ugyanúgy, mint az SL esetében. Például, a mondat $(Pj \eif Mda)$ igaz \model{M}-ben, ha $Pj$ hamis \model{M}-ben vagy $Mda$ igaz \model{M}-ben.
...
Sajnálatos módon, ez a megközelítés kudarcot vall, ha figyelembe vesszük a számszerűsítőket tartalmazó mondatokat. Figyelembe veszi a $\forall x Px$. Mikor igaz ez egy modellben \modell{M}. A válasz nem függ attól, hogy $Px$ igaz vagy hamis a \model{M}-ben, mert az $x$ az $Px$-ben egy szabad változó, $Px$ pedig nem egy mondat. Ez se nem igaz, se nem hamis.
...
Azért tudtuk megadni az igazság rekurzív meghatározását az SL számára, mert az SL minden jól megfogalmazott formulájának van igazságértéke. Ez a QL esetében nem igaz, ezért nem tudtuk meghatározni az igazságot, kezdve és felépítve az atom-mondatok igazságával. Figyelembe kell vennünk azokat az atomképleteket is, amelyek nem mondatok. Ennek érdekében definiáljuk a \emph{elégedettség}; a QL minden jól megfogalmazott képlete elégedett lesz vagy nem lesz elégedett, akkor is, ha nincs igazságértéke. Ezután meg tudjuk határozni a \emph{igazság]-t a QL mondataira az elégedettség szempontjából.




\subsection{Satisfaction}
\subsection{Elégedettség}
...
A $Px$ képlet azt mutatja tömören, hogy $x$ az egyike az $P$-nek. Ez azonban nem teljesen igaz, mert a $x$ változó, és nem állandó. Az UD egyetlen tagját sem nevezi meg. Ehelyett a mondatban szereplő jelentését a számszerűsítő határozza meg, amely azt megköti. A $x$ változónak be kell állnia az UD minden tagjára a $\forall xPx$ mondatban, de elegendő egy tagjának megegyeznie $\exists xPx$-ben. Mivel azt akarjuk, hogy az elégedettség meghatározása a $Px$ -ra kiterjedjen, bármiféle számszerűsítés nélkül, akkor azzal kezdjük, hogy hogyan kell értelmezni egy olyan szabad változót, mint a $x$ $Px$-ben.
